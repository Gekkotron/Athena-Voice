...
"#,
    ...
    fn parses_assist_profile_toml() {
        let repo_root = Path::new(env!("CARGO_MANIFEST_DIR"))
            ...
            .expect("workspace root");
        let cfg =
            load(&repo_root.join("athena.assist.example.toml")).expect("assist profile parses");
    ...
    fn parses_docker_profile_toml() {
        let repo_root = Path::new(env!("CARGO_MANIFEST_DIR"))
            ...
            .expect("workspace root");
        let cfg =
            load(&repo_root.join("athena.docker.example.toml")).expect("docker profile parses");
